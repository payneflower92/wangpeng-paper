...
from parrallel_areaToPoint_poisson_kriging import area_to_point_pk
...
def processAreaID(name, area_id, possible_idx, arr_bl, dict_ps, semivariogram_model, number_of_neighbors, max_range,
                  raise_when_negative_prediction, raise_when_negative_error, err_to_nan):
    returnArr = []
    if area_id in possible_idx:
        nowTime=datetime.now()
        print("start to process area: {}, start time is {} ".format(area_id, nowTime.strftime("%H:%M:%S")))
        ...
        timegap = finishedTime-nowTime
        timegap= str(timegap.total_seconds())+"s"
        print("finish process area: {}, start time is {}, time used: {} ".format(area_id, finishedTime.strftime("%H:%M:%S"), timegap))

        for result in results:
            pred_arr = [area_id, Point(result[0]), result[1], result[2]]
            returnArr.append(pred_arr)

    return (name, returnArr)
...
def smooth_area_to_point_pk(semivariogram_model: TheoreticalVariogram,
    ...
    for area_id in arr_bl[:, 0]:
        taskParamsDic[area_id] = area_id

    processLst = [pool.apply_async(processAreaID, args=(
    name, param, possible_idx, arr_bl, dict_ps, semivariogram_model, number_of_neighbors, max_range,
    raise_when_negative_prediction, raise_when_negative_error, err_to_nan)) for name, param in taskParamsDic.items()]
    resultDic = dict().fromkeys(arr_bl[:, 0])
    for p in processLst:
        r = p.get()
        resultDic[r[0]] = r[1]
