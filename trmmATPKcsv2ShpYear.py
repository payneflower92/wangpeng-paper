...
if __name__ == '__main__':
    ...
    parser.add_argument("--taskPattern", '-t', type=int, required=False, help='插值单任务线程数')
    ...
    outDir = os.path.join(dataRoot, "TRMMtifATPKplusRegularizationTransferToShpFinal")
    ...
    for d in os.listdir(csvDir):
        ...
        for f in os.listdir(os.path.join(csvDir, d)):
            if not re.search(r'(\d+).yealySumPrecipitation.TRMM1KmAfterQHshpMaskAndATPK.smoothedPointMatrixOKatpk.csv$', f):
                continue
            srcFile = os.path.join(csvDir, d, f)
            yearmonth = re.search(r'(\d+).yealySumPrecipitation.TRMM1KmAfterQHshpMaskAndATPK.smoothedPointMatrixOKatpk.csv', f).group(1)
