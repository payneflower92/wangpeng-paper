...
global taskDic, pcName

taskDic = {

    "localhost.localdomain": ("^200[\d]", 36, 2, "/mnt/sata3_storage/jinlinfang/wpThesis/"),
...
def atpkOKProcessParallel(srcFile, tifShpFile, newpath, monthHours):
    ...
    condition = (not os.path.isfile(smoothedFile)) or (not (returnVal != 0 and returnVal >= gdf_pts.shape[0]))
    if condition:
        ...
        for semi_model in theoretical_semivariograms:
            try:
                kriged = kriging(observations=areal_centroids,
                                 ...
                                 number_of_workers=taskDic[pcName][2]
                                 )
...
if __name__ == '__main__':
    ...
    # if socket.gethostname()=="DESKTOP-Q08TLI1":
    #     dataRoot = r"D:\wangpengMasterThesis\reOrganizedData"
    # if socket.gethostname()=="101-1-04-40920":
    #     dataRoot = r"D:\金林芳\wpThesis\reorganizedData"

    # if sys.platform == "linux":
    #     dataRoot = "/mnt/sata3_storage/jinlinfang/wpThesis"
    outDir = os.path.join(dataRoot, "TRMMtifATPKVersion3")
    ...
    for yearmonth in sortedSrcFileDic.keys():
        if not re.search(r'' + taskDic[pcName][0], str(yearmonth)):
            continue
    ...
    # pool = multiprocessing.Pool(multiprocessing.cpu_count() - 80)
    pool = multiprocessing.Pool(taskDic[pcName][1])
