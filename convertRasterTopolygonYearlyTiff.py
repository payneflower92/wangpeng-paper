...
outDir = r"C:\Users\crawl\Desktop\workForWP\wangpeng-master-thesis\reorganizedRawData\TRMMtif2ShpPolygon"
...
for d in os.listdir(srcDir):
    for f in os.listdir(os.path.join(srcDir, d)):
        if not re.search(r'.yealySumPrecipitation.qhShpMasked.tif$', f):
            continue
        year = re.search(r'(\d+).yealySumPrecipitation.qhShpMasked.tif', f).group(1)
        year = int(year)

        newName = re.sub(r'.yealySumPrecipitation.qhShpMasked.tif', '.yealySumPrecipitation.qhShpMasked.TRMMPolygonShp.shp', f)
        xyShpName = re.sub(r'.yealySumPrecipitation.qhShpMasked.tif', '.yealySumPrecipitation.qhShpMasked.TRMMPolygonShp.lonlat.shp', f)
