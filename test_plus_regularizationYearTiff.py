...
def atpkOKProcessParallel(srcFile, tifShpFile, newpath):
    print(tifShpFile)
    print(srcFile)

    yearVal = re.search(r'(\d+).yealySumPrecipitation.qhShpMasked.tif$', srcFile).group(1)
    ...
    filterGdf["TrmmValue"] = filterGdf["TrmmValue"]
    ...
    initialVarFigTitle = str(yearVal)+" yearly_"
    deviationFigPath = modelFilePrefix + "deviationFig.png"
    deviationFigTitle = str(yearVal)+" yearly_Deviation: regularized semivariogram vs theoretical model"
    weightFigPath = modelFilePrefix + "weightFig.png"
    weightFigTitle= str(yearVal)+"_weight change over each iteration"
    finalVarFigPath = modelFilePrefix + "finalVarFig.png"
    finalVarFigTitle = str(yearVal)+" yearly_"
    ...
    print("{}: 总共点数：{}".format(yearVal, len(lonArr) * len(latArr)))
    posindexLst = []
    ...
    if condition:
        ...
        print("{} 没有已经跑完的插值文件 或者文件不合格 现在开始进行插值".format(yearVal))
        # Build a map of interpolated values
        preds_cols = []
        errs_cols = []

        print("{}: model {} 插值开始 ".format(yearVal, 'p ' + semi_model.name[:3] + ' ' + str(
            len(semi_model.lags))))
        try:
            ...
            print("{} 's {} ,{} model interpolation run error ,maybe it's singularity matrix ".format(yearVal,
                                                                                                      semi_model.name,
                                                                                                      semi_model.lags))
        print("{}: model {} 插值结束 ".format(yearVal,
                                              'p ' + semi_model.name[:3] + ' ' + str(len(semi_model.lags))))
        print("{}: 插值结束 开始写入磁盘".format(yearVal))
        gdf_pts.to_csv(smoothedFile, sep="\t")
        print("{}: 写入插值结果结束".format(yearVal))
        return "success"
...
if __name__ == '__main__':
    ...
    for d in os.listdir(tifSrcDir):
        if not re.search(r'yearly$', d):
            continue
        for f in os.listdir(os.path.join(tifSrcDir, d)):
            if not re.search(r'.yealySumPrecipitation.qhShpMasked.tif$', f):
                continue
            srcFile = os.path.join(tifSrcDir, d, f)
            yearValue = re.search(r'(\d+).yealySumPrecipitation.qhShpMasked.tif', f).group(1)
            srcFileDic[int(yearValue)] = (srcFile, d, f)
    ...
    for yearValue in sortedSrcFileDic.keys():

        (srcFile, d, f) = sortedSrcFileDic[yearValue]
        yearValue = str(yearValue)


        newName = re.sub(r'.yealySumPrecipitation.qhShpMasked.tif', '.yealySumPrecipitation.TRMM1KmAfterQHshpMaskAndATPK.tif', f)
        ...
        shpName = yearValue + ".yealySumPrecipitation.qhShpMasked.TRMMPolygonShp.shp"
        tifShpFile = os.path.join(tifShpDir, d, shpName)

        paramsInfoLst.append((srcFile, tifShpFile, newpath))
        if not parallel:
            atpkOKProcessParallel(srcFile, tifShpFile, newpath)
